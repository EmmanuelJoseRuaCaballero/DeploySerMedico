...
export default function AcademicProgress_profesor() {
  // Selects
  // Procedimientos
  ...
  const [estudianteCedula, setEstudianteCedula] = React.useState<number>();
  // Graficos
  // Curva aprendizaje
  ...
  React.useEffect(() => {
    ...
    const cargarDatos = async () => {
      try {
        const [procedimientosRes, curvaAprendizajeRes, estudianteRes] =
          await Promise.all([
            fetch(`${API_URL}/api/procedimientos/`),
            authFetch(`${API_URL}/api/curvaaprendizaje/profesor/`),
            authFetch(`${API_URL}/api/estudiante/`),
          ]);
  ...
  const dataFiltrada = curvaAprendizaje.filter(
    (item) =>
      item.codigo_procedimiento === procedimientoID &&
      item.cedula_estudiante === estudianteCedula,
  );
  ...
  return (
    <DashboardLayout>
      <div className="space-y-2">
                  ...
                  {procedimientos.map((proc) => (
                    <div key={proc.id_procedimientos}>
                      ...
                      {proc.opcion_procedimientos.map((op) => (
                        <div key={op.id_opcion_procedimientos} className="ml-2">
                          {/* SIN SUBOPCIONES */}
        ...
        {/* Estudiantes */}
        <div className="grid gap-2">
          <Label className="space">Estudiantes</Label>

          <Select
            
            value={estudianteCedula ? String(estudianteCedula) : ""}
            onValueChange={(value) => setEstudianteCedula(Number(value))}
          >
            ...
            <SelectContent className="w-[var(--radix-select-trigger-width)] max-h-[300px] overflow-y-auto">
              {estudiantes.map((estudiante) => (
                <SelectItem
                  ...
                  value={String(estudiante.cedula_estudiante)}

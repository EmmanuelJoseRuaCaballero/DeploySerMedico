...
interface UltimasRetroalimentaciones {
...
export default function Dashboard_profesor() {
  const [dataBar, setDataBar] = React.useState<BarData[]>([]);
  const [dataPie, setDataPie] = React.useState<PieData[]>([]);
  const [ultimasRetroalimentaciones, setUltimasRetroalimentaciones] = React.useState<
    UltimasRetroalimentaciones[]
  >([]);
  ...
  React.useEffect(() => {
    ...
    const cargarDatos = async () => {
      try {
        const [estadisticasProcedimientosRes, ultimasRetroalimentacionesRes] =
          await Promise.all([
            authFetch(`${API_URL}/api/estadisticasprocedimientos/profesor/`),
            authFetch(`${API_URL}/api/ultimasretroalimentaciones/profesor/`),
          ]);
        ...
        const ultimasAutoevaluacionesData =
          await ultimasRetroalimentacionesRes.json();
    ...
    <DashboardLayout>
      <div className="space-y-3">
        ...
        <div className="bg-card rounded-xl p-6 shadow-sm border border-border">
          ...
          {/* CONTENT */}
          {ultimasRetroalimentaciones.length === 0 ? (
            /* EMPTY STATE */
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {ultimasRetroalimentaciones.map((item) => (
                <Card
                  id={`${item.id_retroalimentacion}`}
